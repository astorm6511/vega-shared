 ...
 */
export function rootTicker(ticker: string): string {
...
export function priceAt(
  ...
  if (!series) return { price: null, date: null };
  const d = nearestOnOrBefore(series.datesAsc, target, maxGapDays);
  if (!d) return { price: null, date: null };
  return { price: series.byDate.get(d) ?? null, date: d };
}
 ...
 */
export const DEFAULT_MAX_ABS_RETURN_PCT = 5000;
 ...
 */
export function calcRet(
  cur: number | null,
  curDate: string | null,
  hist: number | null,
  histDate: string | null,
  maxAbsPct: number = DEFAULT_MAX_ABS_RETURN_PCT,
): number | null {
  if (cur == null || hist == null || hist === 0) return null;
  if (curDate != null && histDate != null && curDate === histDate) return null;
...
export function buildPriceIndex(
  ...
  for (const r of rows) {
    ...
    if (!tk || r.close_price == null) continue;
