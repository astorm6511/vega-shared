...
test("computeReturnPeriods anchors 1D/7D/1M on the resolved current date, not the nominal 'today', when the price sync lags behind the as-at date", () => {
  ...
  const series = buildPriceIndex(rows).get("GOOG");
  ...
  const periods = computeReturnPeriods(series, cfg);
  const expected = ((341.70 - 341.28) / 341.28) * 100;
  assert.ok(periods.perf_1d != null);
  assert.ok(Math.abs((periods.perf_1d as number) - expected) < 1e-9);
});
